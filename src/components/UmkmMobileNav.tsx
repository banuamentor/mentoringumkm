...
export const UmkmMobileNav: React.FC<UmkmMobileNavProps> = ({ currentTab, onSelectTab }) => {
  ...
  return (
    <>
      {/* Mobile Bottom Navigation Bar (Thumb Zone) - Always visible in viewport */}
      <nav
      ...
      {/* Floating Action Button (FAB) on Tablet & Desktop for instant access */}
      <div className="hidden lg:block fixed bottom-6 right-6 z-40">
        <button
          id="umkm-desktop-fab-quick-sale"
          type="button"
          onClick={() => onSelectTab('umkm-sales-new')}
          className="flex items-center gap-2.5 rounded-full bg-emerald-600 px-5 py-3.5 text-sm font-bold text-white shadow-xl ring-4 ring-emerald-500/20 transition-all hover:bg-emerald-500 hover:scale-105 active:scale-95 cursor-pointer"
          title="Catat Penjualan Cepat (1-Tangan)"
        >
